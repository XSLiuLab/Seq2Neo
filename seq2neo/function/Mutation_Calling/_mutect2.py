...
class Mutect2Commandline:
    def __init__(self, ref, Input, Output, f1r2_gz_file, threads, java_options, Normal_name=None):
        self.ref = ref
        self.Input = Input
        self.Ouput = Output
        ...
        if self.normal_name:
            self.need_files.extend([self.ref, self.Ouput, self.normal_name, self.f1r2, self.threads, self.java_options])
            ...
            self.need_files.extend([self.ref, self.Ouput, self.f1r2, self.threads, self.java_options])
            self.mutect2_cmd = ("gatk Mutect2"
                                + " -I %s" * len(Input)
                                + " -R %s"
                                + " -O %s"
